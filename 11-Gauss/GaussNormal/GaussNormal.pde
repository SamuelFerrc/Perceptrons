  ...
  for (float r = 2.0; r <= 2.0; r += 0.1)
  {
    for (float s = 1.9; s <= 1.9; s += 0.1)
    {
      for (float h = 0.0; h <= 0; h += 5)
      {
        ...
        // ------------------------------------------------------
        println("Normal", mX, mY, count, sumY);
        img.loadPixels();
        for (int i = 0; i < img.width; i++)
        {
          for (int j = 0; j < img.height; j++)
          {
            ...
            if (el >= 0.98 && el <= 1.0)
            {
             img2.set(i, j, color(255, 0, 0));
            }
          }
        }
        println("Não Normal", nMX, nMY, countN, sumNY);
        img2.updatePixels();
        img2.save("Torax/ElipseNormal/a_vm_elipse_normal" + (1391) +
                 "_r==" + r + "_s==" + s + "_h==" + h + ".png");
        img.loadPixels();
        ...
        img.save("Torax/ElipseNaoNormal/a_vm_elipse_centro" + (1391) +
                 "_r==" + r + "_s==" + s + "_h==" + h + ".png");
